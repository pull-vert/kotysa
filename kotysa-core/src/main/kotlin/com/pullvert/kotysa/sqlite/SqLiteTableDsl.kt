...
class SqLiteTableDsl<T : Any>(
     ...
     */
    fun column(dsl: SqLiteColumnDsl<T>.(TableColumnPropertyProvider<T>) -> ColumnBuilder<*>) {
        val sqLiteColumnDsl = SqLiteColumnDsl(dsl)
        val column = sqLiteColumnDsl.initialize(sqLiteColumnDsl)
        addColumn(column)
    }
}
